...
describe('Auth functions', () => {
  ...
  describe('verifyLoggedIn', () => {
    beforeEach(() => {
      mockGetItemStorage.mockResolvedValue(MOCK_TOKEN);
      mockConnectionAPIGet.mockResolvedValue({ name: 'name' });
    });
    it('should call redirect in token empty', () => {
      mockConnectionAPIGet.mockReturnValue(undefined);

      verifyLoggedIn();

      expect(redirect).toHaveBeenLastCalledWith(LoginRoutesEnum.LOGIN);
    });
